...
class WeatherGui:
    ...
    def createGui(self):
        ...
        retrieve_button = ttk.Button(self.frm, text="Retrieve", command=self.retrieveCity)
        ...
        if os.path.exists("cities.txt"):
            with open("cities.txt", "r") as file:
                ...
                for city in cities:
                    if city == "":
                        continue
                    button = tk.Button(text, text=f"{city}", command=lambda c=city: self.chooseCity(c), background="white")
    ...
    def showWeatherInfo(self, city, weather):
        ...
        ttk.Button(weather_frame, text="Close", command=self.weather_window.destroy).grid(column=3, row=1)
    ...
    def getWeatherNow(self, weather, city):
        ...
        if weather_info is None:
            ttk.Label(weather_frame, text=f"{city} Not Found", font=("Helviga", 20)).grid(column=0,row=0)
            ...
            row = 0
            ttk.Label(weather_frame, text=city, font=("Helviga", 12)).grid(column=0,row=row)
            ...
            ttk.Label(weather_frame, text=f"{max_temperature} \N{DEGREE SIGN}C").grid(column=1,row=row)
            row+=1

            ttk.Label(weather_frame, text="Min Temperature").grid(column=0,row=row)
            ttk.Label(weather_frame, text=f"{min_temperature} \N{DEGREE SIGN}C").grid(column=1,row=row)
            row+=1
            ...
            ttk.Label(weather_frame, text=f"{weather_type}: {weather_description}").grid(column=1,row=row)
            row+=1
            ...
            ttk.Label(weather_frame, text=f"Speed: {wind_speed}m/s, Direction: {wind_deg}\N{DEGREE SIGN}").grid(column=1,row=row)
            row+=1
